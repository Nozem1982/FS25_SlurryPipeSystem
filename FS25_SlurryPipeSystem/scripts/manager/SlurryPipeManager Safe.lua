...
function SlurryPipeManager:registerVehicle(vehicle)
    ...
    -- configIndexMatches accepts a single index ("2") or a comma-separated list
    -- ("0,2") and returns true if activeIndex is contained in the set.
    -- If cfgIndexStr is nil OR activeIndex is nil (vehicle has no such config
    -- type) we register unconditionally — preserves prior behaviour.
    local function configIndexMatches(cfgIndexStr, activeIndex)
        if cfgIndexStr == nil then return true end
        if activeIndex == nil then return true end
        for token in string.gmatch(cfgIndexStr, "([^,]+)") do
            local idx = tonumber(token)
            if idx ~= nil and idx == activeIndex then
                return true
            end
        end
        return false
    ...
    while true do
        ...
        if cfgIndexStr ~= nil then
            ...
            -- Only skip if the config type exists on this vehicle AND the
            -- entry's index list does not contain it. If activeIndex is nil
            -- the vehicle has no such config — register unconditionally.
            if not configIndexMatches(cfgIndexStr, activeIndex) then
                armIndex = armIndex + 1
                continue
            end
        end
    ...
    while true do
        ...
        if cCfgIndexStr ~= nil then
            ...
            local activeIndex = getActiveConfigIndex(cfgType)
            if not configIndexMatches(cCfgIndexStr, activeIndex) then
